...
def inference(model1, model2, tokenizer1, tokenizer2, target_data, prefix, accelerator, num_shots, ex):
    ...
    for ratio in [0.9]:
        ...
        topk = np.sort(token_log_probs.cpu().to(torch.float32).numpy())[:k_length]
        pred[f"mink"] = np.mean(topk).item()
        
    ## mink++
    mink_plus = (token_log_probs - mu) / sigma.sqrt()
    for ratio in [0.1]:
        k_length = int(len(mink_plus) * ratio)
        topk = np.sort(mink_plus.cpu().to(torch.float32).numpy())[:k_length]
        pred[f"mink++"] = np.mean(topk).item()
...
if __name__ == "__main__":
    ...
    nonmember_prefix = nonmember_prefix[:int(num_shots)]
    ...
    all_output_path = os.path.join(output_dir, f"{dataset}", f"{target_model.split('/')[-1]}", f"{sub_dataset}", f"{num_shots}_shot_{str(sub_dataset)}.csv")
